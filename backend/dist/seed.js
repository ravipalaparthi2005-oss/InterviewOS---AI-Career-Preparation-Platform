...
async function main() {
    ...
    await db_1.default.systemAnalytics.deleteMany({});
    await db_1.default.portfolio.deleteMany({});
    await db_1.default.roadmap.deleteMany({});
    await db_1.default.mockInterview.deleteMany({});
    await db_1.default.jobMatch.deleteMany({});
    await db_1.default.codeSubmission.deleteMany({});
    await db_1.default.resume.deleteMany({});
    await db_1.default.user.deleteMany({});
    await db_1.default.codingProblem.deleteMany({});
    ...
    // Recruiter User
    const recruiter = await db_1.default.user.create({
        data: {
            name: 'Sarah Jenkins',
    ...
    const student2 = await db_1.default.user.create({
        data: {
            name: 'Priya Sharma',
    ...
    const student3 = await db_1.default.user.create({
        data: {
            name: 'David Chen',
    ...
    const resume1 = await db_1.default.resume.create({
        data: {
            userId: student.id,
    ...
    const resume2 = await db_1.default.resume.create({
        data: {
            userId: student2.id,
    ...
    const resume3 = await db_1.default.resume.create({
        data: {
            userId: student3.id,
    ...
    await db_1.default.jobMatch.create({
        data: {
            resumeId: resume1.id,
    ...
    await db_1.default.mockInterview.create({
        data: {
            userId: student.id,
            type: 'TECHNICAL',
            status: 'COMPLETED',
    ...
    for (const log of analyticsData) {
        await db_1.default.systemAnalytics.create({ data: log });
    }
    console.log('🌱 Seeding database with coding problems...');
    for (const problem of SAMPLE_PROBLEMS) {
        await db_1.default.codingProblem.create({
            data: problem
        });
