...
if (process.env.GEMINI_API_KEY) {
    try {
        ai = new generative_ai_1.GoogleGenerativeAI(process.env.GEMINI_API_KEY);
    }
    catch (error) {
...
class GeminiService {
     ...
     */
    static async callGemini(prompt, fallbackData) {
        if (!ai) {
     ...
     */
    static async analyzeResume(resumeText, jobDesc) {
        const prompt = `
     ...
     */
    static async generateRoadmap(skills, targetJob) {
        const prompt = `
     ...
     */
    static async generateInterviewQuestion(type, history, company) {
        const prompt = `
     ...
     */
    static async evaluateInterviewAnswer(question, answer, type) {
        const prompt = `
     ...
     */
    static async generateCodingHints(problemTitle, problemDescription, userCode, errorMessage) {
        const prompt = `
