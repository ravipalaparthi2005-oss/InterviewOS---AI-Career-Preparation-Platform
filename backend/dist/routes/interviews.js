...
router.post('/start', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const qData = await gemini_1.GeminiService.generateInterviewQuestion(type, [], companySpecific);
        // Save initial interview record
        const interview = await db_1.default.mockInterview.create({
            data: {
                userId,
                type,
                status: 'PENDING',
...
router.post('/submit-answer', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const interview = await db_1.default.mockInterview.findUnique({ where: { id: interviewId } });
        if (!interview) {
            return res.status(404).json({ error: 'Interview session not found' });
        }
        // Evaluate answer with Gemini
        const evaluation = await gemini_1.GeminiService.evaluateInterviewAnswer(question, answer, interview.type);
        ...
        await db_1.default.mockInterview.update({
...
router.post('/complete', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const completed = await db_1.default.mockInterview.update({
            where: { id: interviewId },
            data: {
                status: 'COMPLETED',
                score: finalScore || 85,
...
router.get('/history', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        const history = await db_1.default.mockInterview.findMany({
            where: { userId },
