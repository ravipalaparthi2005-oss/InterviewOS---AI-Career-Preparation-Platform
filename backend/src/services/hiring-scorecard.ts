...
export interface HiringScorecard {
...
export class HiringScoreCard {
  static async generateScorecard(
    ...
    if (overallScore < 40) hiringProbability = Math.max(5, overallScore - 20);
    else if (overallScore > 85) hiringProbability = Math.min(99, overallScore + 10);

    // Determine recommendation
    let recommendation: 'STRONG_YES' | 'YES' | 'MAYBE' | 'NO' | 'STRONG_NO';
    if (hiringProbability >= 85) recommendation = 'STRONG_YES';
    else if (hiringProbability >= 70) recommendation = 'YES';
    else if (hiringProbability >= 55) recommendation = 'MAYBE';
    else if (hiringProbability >= 40) recommendation = 'NO';
    ...
    const skillAssessment = skills.map(skill => ({
      skill,
      level: this.getSkillLevel(baseScores.technical) as 'beginner' | 'intermediate' | 'advanced' | 'expert',
  ...
  private static getTypeBasedFeedback(interviewType: string) {
  ...
  private static getNextSteps(recommendation: string): string[] {
