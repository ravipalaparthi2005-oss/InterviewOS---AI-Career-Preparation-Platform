...
const mentorSessions = new Map<string, Array<{ role: 'user' | 'mentor'; text: string }>>();
...
Be concise, actionable, and specific. Provide concrete examples when possible.`;
    ...
    const conversationContext = context ? context.map((msg: any) => `${msg.role}: ${msg.text}`).join('\n') : '';
...
Provide a helpful, concise mentor response (2-3 sentences max):`;
    ...
    if (GeminiService) {
      try {
        const model = (global as any).ai?.getGenerativeModel?.({ model: "gemini-1.5-flash" });
        if (model) {
...
router.post('/clear-session', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
